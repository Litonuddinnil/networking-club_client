...
export default function CommandPalette({ isOpen, onClose, onNavigate }: CommandPaletteProps) {
  ...
  const commands = [
    { id: "dashboard", label: "Go to Dashboard Console", icon: <Layers className="w-4 h-4 text-emerald-400" />, action: () => { onNavigate("dashboard"); onClose(); } },
            ...
            filteredCommands.map((cmd) => (
              ...
              >
                <div className="flex items-center space-x-3">
                  <div className="p-2 bg-slate-900 border border-white/5 rounded-lg group-hover:border-emerald-500/30 transition-colors">
                    {cmd.icon}
                  </div>
                  <span>{cmd.label}</span>
                </div>
                <span className="text-[9px] text-slate-600 group-hover:text-emerald-400">Execute ↵</span>
              </button>
            ))
          )}
        </div>
