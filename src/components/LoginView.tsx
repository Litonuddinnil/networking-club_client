  ...
  return (
    <div className="min-h-screen bg-[#020408] text-slate-300 flex items-center justify-center p-4 md:p-8 relative font-sans overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-5xl grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-10 items-stretch">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl font-extrabold text-white tracking-tight">
                  JSTU <span className="text-emerald-400">NetClub</span>
                </h2>
            ...
            {lines.map((l, i) => (
              ...
              >
                <div className="w-8 h-8 rounded-lg bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center">
                  <l.icon size={14} className="text-emerald-400" />
                </div>
        ...
        <div
          className={`relative gradient-border rounded-3xl p-8 md:p-10 bg-[#060b14]/80 backdrop-blur-xl shadow-2xl shadow-emerald-500/10 ${
            shake ? "animate-shake" : ""
          }`}
        >
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div className="space-y-1.5">
              ...
              <div className="relative flex items-center group">
                <User className="absolute left-3 w-4 h-4 text-slate-500 group-focus-within:text-emerald-400 transition" />
                <input
                  type="text"
                  value={username}
                  onChange={(e) => setUsername(e.target.value)}
                  placeholder="JNC-XXXX-XXXX"
                  className="w-full bg-slate-900/80 border border-white/10 text-white pl-10 pr-4 py-3 rounded-xl outline-none focus:border-emerald-500/60 focus:ring-2 focus:ring-emerald-500/10 text-xs font-mono transition"
                  required
                />
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative flex items-center group">
                <Lock className="absolute left-3 w-4 h-4 text-slate-500 group-focus-within:text-emerald-400 transition" />
                <input
                  type={showPass ? "text" : "password"}
                  value={password}
                  onChange={(e) => setPassword(e.target.value)}
                  placeholder="••••••"
                  className="w-full bg-slate-900/80 border border-white/10 text-white pl-10 pr-10 py-3 rounded-xl outline-none focus:border-emerald-500/60 focus:ring-2 focus:ring-emerald-500/10 text-xs font-mono transition"
                  required
                />
                <button
                  type="button"
                  onClick={() => setShowPass((s) => !s)}
                  className="absolute right-3 text-slate-500 hover:text-emerald-400 transition"
                >
            ...
            <div className="flex items-center justify-between py-1">
              <label className="flex items-center gap-2 text-slate-500 font-semibold select-none cursor-pointer text-xs">
                <input
                  ...
                  className="rounded border-white/10 text-emerald-500 focus:ring-0 bg-transparent w-3.5 h-3.5"
                />
                <span>Remember me</span>
              </label>
              <a href="#" className="text-emerald-400 hover:text-emerald-300 font-semibold text-xs">
                Forgot password?
              </a>
            </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className="group relative w-full py-3.5 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-60 text-white font-bold text-xs rounded-xl shadow-lg shadow-emerald-600/30 transition-all overflow-hidden mt-2"
            >
